  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      input { width:260px; padding:11px 12px; border:1px solid var(--line); border-radius:12px; background:#fffdf8; color:var(--ink); font:14px ui-sans-serif, system-ui, sans-serif; }
    ...
    <main>
      <header>
        <div>
          ...
          <h1>Backend pulse.</h1>
          <p class="sub">A read-only operational view for users, visits, renders, and active devices. Pageview history starts from the moment analytics tracking is enabled.</p>
        </div>
        <div class="auth">
      ...
      <p id="status" class="status">Enter ADMIN_TOKEN to load protected metrics. Device APIs remain available under /api/*.</p>
      <section id="metrics" class="grid" aria-live="polite">
        ...
        <article class="card"><p class="metric-label">Visits Today</p><p id="visitsToday" class="metric-value">--</p><p id="visitsNote" class="metric-note">pageviews</p></article>
        <article class="card"><p class="metric-label">Active Devices</p><p id="devicesActive" class="metric-value">--</p><p id="devicesNote" class="metric-note">last 24h</p></article>
        <article class="card"><p class="metric-label">Renders</p><p id="rendersTotal" class="metric-value">--</p><p id="rendersNote" class="metric-note">total</p></article>
        <article class="card wide"><p class="metric-label">Visit Trend</p><div id="visitBars" class="bars"><p class="empty">No pageview data yet.</p></div></article>
        <article class="card wide"><p class="metric-label">Top Paths 7d</p><div id="topPaths" class="empty">No pageview data yet.</div></article>
        <article class="card wide"><p class="metric-label">Recent Devices</p><div id="recentDevices" class="empty">No device activity yet.</div></article>
        <article class="card wide"><p class="metric-label">Notes</p><p id="historyNote" class="metric-note">Waiting for data.</p><p class="metric-note" style="margin-top:10px">Useful links: <a href="/api/health">API health</a> / <a href="/static/art/moyun.png">static art probe</a></p></article>
      </section>
    </main>
    <script>
      const $ = (id) => document.getElementById(id);
      const tokenInput = $("token");
      tokenInput.value = localStorage.getItem("inksight_admin_token") || "";
      ...
      function renderBars(days) {
        if (!days || days.length === 0) {
          $("visitBars").innerHTML = '<p class="empty">No pageview data yet.</p>';
          return;
        }
        const max = Math.max(...days.map((d) => d.count || 0), 1);
        $("visitBars").innerHTML = days.map((d) => {
          const h = Math.max(8, Math.round((d.count || 0) / max * 110));
          const label = String(d.date || "").slice(5);
          return `<div class="bar" title="${d.date}: ${d.count}" style="height:${h}px"><span>${label}</span></div>`;
        }).join("");
      }

      function render(data) {
        $("usersTotal").textContent = fmt(data.users.total);
        $("usersNote").textContent = `+${fmt(data.users.today)} today / ${fmt(data.users.active_7d)} active 7d`;
        $("visitsToday").textContent = fmt(data.visits.today);
        $("visitsNote").textContent = `${fmt(data.visits.total)} total / ${fmt(data.visits.unique_today)} unique today`;
        $("devicesActive").textContent = fmt(data.devices.active_24h);
        $("devicesNote").textContent = `${fmt(data.devices.total)} total / ${fmt(data.devices.active_7d)} active 7d`;
        $("rendersTotal").textContent = fmt(data.renders.total);
        $("rendersNote").textContent = `+${fmt(data.renders.today)} today`;
        $("historyNote").textContent = data.visits.history_note || "";
        renderBars(data.visits.daily);
        $("topPaths").innerHTML = renderTable(data.visits.top_paths_7d, [{ key: "path", label: "Path" }, { key: "count", label: "Views" }]);
        $("recentDevices").innerHTML = renderTable(data.devices.recent, [{ key: "mac", label: "MAC" }, { key: "last_seen", label: "Last seen" }]);
        setStatus(`Loaded at ${new Date(data.generated_at).toLocaleString()}.`);
      }

      async function loadConsole() {
        const token = tokenInput.value.trim();
        if (!token) {
          setStatus("ADMIN_TOKEN is required to load metrics.", true);
          return;
        }
        localStorage.setItem("inksight_admin_token", token);
        setStatus("Loading metrics...");
        try {
          const res = await fetch("/api/admin/console/summary", {
      ...
      $("load").addEventListener("click", loadConsole);
      $("clear").addEventListener("click", () => {
        localStorage.removeItem("inksight_admin_token");
        tokenInput.value = "";
        setStatus("Token cleared.");
      });
      tokenInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter") loadConsole();
      });
      ...
      if (tokenInput.value) loadConsole();
    </script>
  </body>
</html>"""
